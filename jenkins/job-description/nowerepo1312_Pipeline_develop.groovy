...
def job_config = [
        job: [
                name: "nowerepo1312_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'nowerepo1312_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowerepo1312_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.nowerepo1312.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "nowerepo1312", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("nowerepo1312_Pipeline_develop");

job.with {

    authenticationToken('Af5roWRPk2FaDHhMxxBMyzFuguQzJXRk')
